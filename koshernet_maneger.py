...
RESTRICT_CHROME='''URLBlocklist.reg'''
UNRESTRICT_CHROME='''Cancel_URLBlocklist.reg & powershell -Command "Start-Process cmd -Verb RunAs -ArgumentList '/c netsh advfirewall set allprofiles firewallpolicy blockinbound,blockoutbound & netsh advfirewall firewall add rule name=\\\"chrome1\\\" program=\\\"C:\Program Files\Google\Chrome\Application\chrome.exe\\\" dir=out action=allow & netsh advfirewall firewall add rule name=\\\"chrome2\\\" program=\\\"C:\Program Files (x86)\Google\Chrome\Application\chrome.exe\\\" dir=out action=allow'"'''
APPLY_WHITE_LIST='''URLAllowlist.reg'''
def execute(command):
    os.system(command)
